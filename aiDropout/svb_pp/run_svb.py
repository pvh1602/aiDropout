...
import model_svb, model_svb
import utilities
from shutil import copyfile


all_dataset_dir = '../datasets'
model_folder = 'result_svb'

def main():
	# check input
	if len(sys.argv) != 3:
		print ('Usage: python run_svb.py [rho] [times]')
		exit()
	...
	data_result = '/'.join([model_folder, 'rho_' + str(rho), 'result_' + times])
	if not os.path.exists(data_result):
		os.makedirs(data_result)

	MODEL = model_svb.Model(num_topic, n_term, batch_size, n_infer, alpha, rho)
