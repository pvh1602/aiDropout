...
all_model_folder = 'result_dropout_v2'

def main():
	# check input
	if len(sys.argv) != 6:
		print ('Usage: python run_dropout.py [sigma] [rate] [times] [type_model] [iters]')
		exit()
	...
	if (type_model == 'B'):
		MODEL = model_LDA.model(num_topic, n_term, batch_size, n_infer, learning_rate, alpha, sigma, rate, 0, iters)
	elif (type_model == 'S'):
		MODEL = model_LDA.model(num_topic, n_term, batch_size, n_infer, learning_rate, alpha, sigma, rate, 1, iters)
	elif (type_model == 'Z'):
		MODEL = model_LDA.model(num_topic, n_term, batch_size, n_infer, learning_rate, alpha, sigma, rate, 2, iters)
	else:
		print ('Unknown type model !!!')
		exit()
